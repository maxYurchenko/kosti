...
exports.getTimePassedSincePostCreation = function (postCreationDate) {
  ...
  if (
    typeof postCreationDate === "string" ||
    postCreationDate instanceof String
  ) {
    postCreationDate = new Date(postCreationDate);
  }

  var minutesPassed = getMinutesDifference(postCreationDate);
  if (minutesPassed === 0) return "меньше минуты назад";
...
function getWordNumber(num) {
  ...
  if (num % 10 === 1 && num % 1000 != 11) return 0;
  //for numbers that end with '2', '3', '4' but not '12', '13', '14'
  else if (
    (num % 10 === 2 && num % 1000 != 12) ||
